...
var distributeTranscludeDirective = function() {
    return {
        ...
        link: function(scope, elem, attr, ctrl, transclude) {
            transclude(scope, function(clone) {
                var transcludeSelectors = elem[0].querySelectorAll('[trans-select]');
                angular.forEach(clone, function(cloneEl) {
                    var found = false;
                    for (var i = transcludeSelectors.length; i--;) {
                        var transcludeToElement = transcludeSelectors[i];
                        if (isAllowedNode(cloneEl) && matches(cloneEl, transcludeToElement.attributes['trans-select'])) {
    ...
    function matches(element, selector) {
        var matchesFunc = element.matches || element.matchesSelector;
        if (element.matches) {
            return element.matches(selector.value);
        } else if (element.matchesSelector) {
            return element.matchesSelector(selector.value);
        } else if (element.querySelectorAll) {
            return supportOlderBrowser(selector.value);
        } else {
    ...
    function supportOlderBrowser(selector) {
        var element = this;
